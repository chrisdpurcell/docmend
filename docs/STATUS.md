...
- The Plan D implementation baseline is 983 passing tests plus one opt-in 100k-file scale skip, with 95% branch coverage.
...
- Plan D was implemented as eight green commits on `plan-d-verify-redesign` after three audit rounds approved the [plan](superpowers/plans/2026-07-11-safety-core-d-verify-redesign.md). `verify --plan` now certifies repeatable attempt lineage and exactly-once outcomes; restore evidence is lifecycle-only and never requires an apply report.
