...
| NFR-001 | DMR-08 qualification suite: 1,000-file source-tree regression in the default gate; deterministic evidence-model and fitting unit tests; accepted external uninstrumented 100,000-file pilot/scheduled evidence; accepted file-size matrix; accepted installed-wheel 1,000,000-file evidence satisfying corpus conservation, expected findings, peak/slope/linearity thresholds, reference environment, and 12-hour runtime. The historical `tests/test_scale.py` tracemalloc run is non-binding and will be replaced. | Partial (revision-one contract approved; harness, pilot, revision-two thresholds, and 1M evidence pending) |
...
| IR-006 | Existing strict-validation tests plus DMR-08 migration cases in `tests/test_config.py` and `tests/test_cli_scan.py`: default/empty config accepted; every `[parallel]` shape rejected before scanning with the migration-specific exit-2 message; unknown/type/enum/range errors remain strict. | Partial (current strict loader complete; v2 parallel-table rejection pending DMR-08 Task 2) |
...
| DR-002 | Existing plan schema/action/source-hash coverage plus DMR-08 plan 2.0 model/schema tests, absence of the legacy snapshot field, supported-inventory reuse, and pre-gate/pre-mutation 1.x rejection in `tests/test_plan_artifact.py`, `tests/test_cli_apply.py`, and `tests/unit/writer/test_commit.py`. | Partial (v1 plan contract complete; plan 2.0 migration pending DMR-08 Task 2) |
