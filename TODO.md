...
- [x] Created `repo-hygiene.md` with a repository hygiene checklist and linked it from `AGENTS.md`.
...
- [x] **ADR candidate assessment + Tier-1 ADRs + `docs/adr` reorg (2026-07-05):** reviewed spec D-001..009 + RQ-001..023 → prioritized 12-ADR backlog (`docs/adr/adr-backlog.md`, 4 Tier-1 / 4 Tier-2 / 4 Tier-3, 7 deliberate skips). Drafted the 4 Tier-1 ADRs — 0002 layered pipeline, 0003 in-place output model, 0004 safety gate, 0005 artifact contract. Moved the ADR folder `docs/decisions/` → `docs/adr/` (git-mv, all real references updated), added `README.md` + `index.md`, wired ADR↔spec/RQ cross-links. Repo markdownlint clean (0 errors). Completes the owner's five ADR-structure tasks + the "Assess ADR candidates" task.
