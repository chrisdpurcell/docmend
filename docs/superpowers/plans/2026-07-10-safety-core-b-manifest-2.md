...
- Modify: `src/docmend/cli.py` (`_read_resume_records` reads via `read_manifest_set`; restore reads via `read_manifest_set`; clean-break message surfaces as exit 2)
- Create: `tests/helpers/manifest2.py` (header/record/set fixture builder used by every manifest test)
- Test: `tests/unit/writer/test_manifest.py`, plus mechanical fixture updates across `tests/test_restore.py`, `tests/test_resume.py`, `tests/test_cli_apply.py`, `tests/test_cli_restore.py`, `tests/test_restore_drill.py`
...
- **Attempt-lineage consistency (CR-006), validated where locally provable:** the root set carries `prior_attempt: null` AND `prior_manifest_sha256: null` together; a non-root set carries `prior_attempt` non-null; when a set's `prior_attempt` carries `manifest_sha256`, that hash must EQUAL the set's own `prior_manifest_sha256` (the predecessor's newest manifest is the attempt evidence) and the named `run_id` must be that predecessor set's; when it carries `report_sha256`, the named `run_id` must NOT be any manifest in the chain newer than `prior_manifest_sha256`'s set (a report-only attempt has no manifest), and `prior_manifest_sha256` must still name the newest earlier manifest — the subchain and attempt chain cannot disagree. Report-hash verification itself (does `report_sha256` match a real report file?) is DEFERRED: apply-resume validates it at edge-build time (Task 9) because it holds the report input; verify (Plan D) validates it chain-wide with report inputs. This deferral is deliberate and stated here so Plan D picks it up.

- [ ] **Step 1: Failing tests** — shuffled three-file chain orders deterministically; forked chain rejected; gap rejected; wrong `prior_manifest_sha256` hash rejected; mixed `plan_sha256` rejected; apply-after-restore rejected; restore record undoing an unknown action rejected; `applied` then contradictory `failed` in a later apply set rejected; root with non-null `prior_attempt` but null `prior_manifest_sha256` when a predecessor manifest was supplied rejected; `prior_attempt.manifest_sha256` disagreeing with `prior_manifest_sha256` rejected; self-run reference rejected; duplicate `run_id` rejected; empty input → empty chain.
...
**Interfaces (CR-004 — predecessor discovery is a SEPARATE input from mutation-manifest loading):**

- `--resume-run-id RID` resolves BOTH default sidecars: `.docmend/docmend-RID-manifest.jsonl` AND `.docmend/docmend-RID-report.json`. The manifest feeds the mutation chain; the report feeds the lineage edge.
- New repeatable `--prior-report PATH` names a relocated or report-only predecessor report (design CLI discovery). A report-only predecessor (report exists, no manifest — e.g., a first-action abort that never mutated) is LEGAL: the chain may be empty while `prior_attempt` carries the report's sha256.
- `_load_predecessor_attempt(resume_run_ids, prior_reports) -> tuple[PriorAttempt | None, str | None]` returns the lineage edge for the NEWEST predecessor attempt plus `prior_manifest_sha256` (the newest predecessor manifest's hash, or None when no predecessor manifest exists): predecessor report present → `PriorAttempt(run_id, report_sha256=sha_of_report, manifest_sha256=None)` after validating the report parses as a 2.0 report and its `run_id` matches; report absent but manifest present → `PriorAttempt(run_id, report_sha256=None, manifest_sha256=manifest_sha256(path))`; neither → exit 2 (the named predecessor left no evidence).
- Apply computes `plan_sha256 = f"sha256:{hashlib.sha256(plan_path.read_bytes()).hexdigest()}"`, passes it + `prior_manifest_sha256` + `prior_attempt` + `backup_root` into `execute_plan`; after the run, stamps `manifest_sha256` (null when no manifest was created) and the same `prior_attempt` onto the report — both artifacts carry the identical edge (design F6 round 5) — then writes it inside the lock as today.

- [ ] **Step 1: Failing tests** — a fresh write apply's header carries the plan file's sha256, `prior_attempt: null`; a `--resume-run-id` apply's header carries `prior_manifest_sha256` = predecessor manifest hash and `prior_attempt.report_sha256` = predecessor report hash, with the report repeating the identical edge; a REPORT-ONLY predecessor (delete the predecessor manifest, keep its report): resume succeeds with an empty chain and `prior_attempt.report_sha256` set; a relocated report via `--prior-report` works; a predecessor with NEITHER sidecar exits 2; resuming from a 1.x manifest exits 2 with the clean-break message; the composed three-attempt lineage — R1 report-only → R2 manifest-with-missing-report → R3 success — connects: R3's `prior_attempt` names R2 by manifest hash, R2's header names R1 by report hash (the design's principal interruption shape).
...
- `run_restore(chain: ManifestChain, *, run_id, write, only_ids, manifest_out) -> list[RestoreOutcome]`:
  ...
  - Writer header: kind `restore`, `source_root`/`plan_sha256`/`backup_root` copied from the chain, `prior_manifest_sha256` = tip manifest's hash, `prior_attempt` = the tip attempt's edge (its run_id + manifest hash).
...
- **Review closure mapping:** CR-001 → `lineage.py` + `tests/test_lineage_imports.py` (Tasks 1, 5, 8 import from it); CR-002 → Task 2 rule 7 with the six adversarial filesystem fixtures; CR-003 → the green-slice constraint + every task's "FULL gate" step (the format break is one slice, Task 2); CR-004 → Task 9's `_load_predecessor_attempt`, `--prior-report`, empty-chain resume, and the composed three-attempt regression; CR-005 → Task 10's `RESTORE_CRASH_MATRIX` (one row per design-table restore row) + restore identity probes, with the adjudicator's restore rows unit-tested in Task 7; CR-006 → Task 3's attempt-lineage invariants with the report-hash deferral stated; CR-007 → adr-0019 in Global Constraints.
...
- **Restore header `backup_root`:** copied from the chain (the apply run's root) because restore READS those backups; its own inverses reference them in `undoes` context only. This matches the header's per-run meaning: the root the SET's backup references resolve under. Flagged per the review's verify-before-correcting list.
