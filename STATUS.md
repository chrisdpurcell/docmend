...
- Spec is `status: draft`, `full` profile. After the 2026-07-05 owner decisions, **three** blocking open questions remain (OQ-018 → MS-1, OQ-015 → MS-2, OQ-012 → MS-3 write path); ten questions are settled in `docs/resolved-questions.md` (RQ-001..010). The full ranked gap landscape is in `docs/gap-analysis.md`.
