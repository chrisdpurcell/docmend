...
**Files (round-3 CR-NEW-005 — EVERY live Report/ReportTotals constructor changes in this slice):**
...
- Modify: `src/docmend/writer/apply.py` (abort accounting + `execute_plan`'s `Report`/`ReportTotals` construction gains `not_attempted`, `prior_attempt=None`, `manifest_sha256=None` — the CLI stamps real values in Task 9)
- Modify: `src/docmend/cli.py` (`_write_refusal_report` ~`:714` constructs both models directly — gains `not_attempted=0`, `prior_attempt=None`, `manifest_sha256=None`)
- Test: `tests/test_apply.py`, `tests/test_report_artifact.py`, `tests/test_verify.py` (both construct Report models directly), `tests/test_schemas.py` (`_minimal_report` gains the required 2.0 wire members)
...
- [ ] **Step 1: Failing tests** — abort mid-plan (collision `fail` on action 2 of 3) yields outcomes `[applied, skipped, not-attempted]` and `totals.not_attempted == 1`; the 2.0 schema validates it and rejects totals omitting `not_attempted`; a refused write's refusal report (`_write_refusal_report`) carries the complete 2.0 shape — `not_attempted` totals, `prior_attempt: null`, `manifest_sha256: null` — and validates against the schema (CR-NEW-005's first-action refusal assertion).
- [ ] **Step 2: Verify failure.** → **Step 3: Implement** (`if abort: outcomes.extend(...)` over `plan.actions[len(outcomes):]`; every constructor site from the Files list updated). → **Step 4: FULL gate.** → **Step 5: Commit** — `git add src/docmend/report.py src/docmend/schemas/report.schema.json src/docmend/artifacts.py src/docmend/writer/apply.py src/docmend/cli.py tests/test_apply.py tests/test_report_artifact.py tests/test_verify.py tests/test_schemas.py && git commit -m "feat(report): 2.0 — attempt lineage, not-attempted partition, totals"`
...
- `_load_predecessor_attempt(resume_manifests, resume_run_ids, prior_reports, *, plan_sha256) -> tuple[PriorAttempt | None, str | None]` builds the ATTEMPT GRAPH and returns the tip's lineage edge plus `prior_manifest_sha256`:
  ...
  - **Edges:** every node's `prior_attempt` reference. **NO-GAP rule (round-3 CR-006):** every non-null edge must resolve — by BOTH the referenced artifact hash (report sha256 or manifest sha256) AND `run_id` — to exactly ONE supplied node; zero matches (a named ancestor whose evidence was not supplied, including an absent report-only predecessor) or multiple matches → exit 2 before any mutation, naming the unresolvable edge. **Tip selection is graph-derived, never caller-order:** the tip is the unique node no other node references as its predecessor; zero or multiple tips → exit 2 with the ambiguity named.
...
- **Review closure mapping (rounds 1–2):** CR-001 → `lineage.py` + `tests/test_lineage_imports.py` (Tasks 1, 5, 8 import from it); CR-002 → Task 2 rule 7 with the six adversarial filesystem fixtures; CR-003 → the green-slice constraint, the complete consumer inventory in Task 2 (verify path + the three test files round 2 found missing), and the PROVISIONAL set rule that keeps pre-Task-6 producers readable; CR-004 → Task 3's report-flavored-root legality + Task 9's attempt graph over manifests/run-ids/reports with graph-derived tip selection; CR-005 → Task 10's `RESTORE_CRASH_MATRIX` + restore identity probes, adjudicator restore rows unit-tested in Task 7; CR-006 → Task 3's lineage invariants + Task 9's edge-build report validation (`plan_ref` binding, `manifest_sha256` reconciliation, report `prior_attempt` graph consistency), with only genuine Plan-D checks deferred; CR-007 → adr-0019 in Global Constraints; CR-NEW-001 → Task 9's report-only vs missing-manifest distinction (`manifest_sha256: null` + zero applied required for an empty chain); CR-NEW-002 → Task 2 provisional terminals + Task 3 chain-closure rule with isolated-set rejection tests, AND (round 3) the approved design's per-set paragraph amended in the same commit so the design, adr-0019, and this plan state one lifecycle contract; CR-NEW-003 → Task 10 restore header `backup_root: null` + direct assertion; CR-NEW-004 → the review artifact is tracked; CR-NEW-005 → Task 8's complete constructor inventory with explicit staging and the refusal-report 2.0-shape assertion; CR-006 (round 3) → the attempt graph's no-gap rule with the missing-ancestor exit-2 test.
