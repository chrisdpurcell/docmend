...
| FR-011 | The system shall detect target-path collisions and resolve them per the configured policy: `skip` (default), `fail`, or `overwrite`. A plan-internal conflict — two actions in the same run claiming one target — is never resolvable by `overwrite`: the later action skips with reason under every policy. | `foo.txt` → `foo.md` where `foo.md` exists must be an explicit decision, not an accident; and no policy may let one planned action silently destroy another action's output (G-005). | Collision fixture under each policy yields skip-with-reason, non-zero abort, or manifest-recorded overwrite respectively; a same-run claim fixture (`a.TXT` + `a.txt` → `a.md`) yields one action + one collision skip under all three policies. | Must |
...
| DR-004 | Backup/rename manifest | The system shall persist a reversible operation record per mutation: original path, target path, backup path (if any), before/after hashes, operation type, result status, error details. A multi-step mutation (`rename_and_rewrite`) additionally persists a write-ahead **intent** record (expected after-hash; clobbered-target hash under overwrite) before its first step, so a hard kill inside the step window leaves reconcilable evidence for resume (adr-0006 amendment). | JSON schema (OQ-004); sufficient to mechanically restore the pre-apply state; restore replays and verify reconciles only `applied` records. | docmend |
...
| IR-008 | `restore` drill test: manifest replay restores pre-apply hashes — `tests/test_restore.py`, `tests/test_restore_drill.py`. | Complete (MS-3) |
