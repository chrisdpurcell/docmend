...
| IR-007 | Artifact round-trip tests (JSON documents; NDJSON manifest per record). Inventory half at MS-1: write→read→identical model, atomicity, ERR-008 failure modes — `tests/test_inventory_artifact.py`; plan half at MS-2 — `tests/test_plan_artifact.py`, `tests/test_schemas.py`; destination guard, two-candidate containment, `.docmend/` carve-out, and exclusive staging — `tests/test_artifact_guard.py`, `tests/test_artifact_containment.py`. | Complete (MS-3 v1 contract; safety-core Plan A guarded artifact publication) |
