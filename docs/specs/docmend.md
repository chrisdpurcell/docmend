...
Scale evidence is versioned JSON under `docs/scale-evidence/`, separate from the product `ArtifactKind` registry. An accepted evidence document binds the candidate commit and exact wheel hash; package, Python, kernel, and schema versions; reference-environment hash; corpus count and bytes; per-stage elapsed time, throughput, external peak RSS, artifact sizes, and reconciled outcome totals; and threshold results. Public evidence rejects hostnames, usernames, absolute paths, serial/device identifiers, document content, and unknown or value-bearing mount options. Supporting pilot points live under `docs/scale-evidence/supporting/`; only complete reviewed baselines live under `docs/scale-evidence/accepted/`, and acceptance never overwrites an existing file. `docs/scale-evidence/thresholds.json` is the executable revision-two source for the frozen peak, slope, and linearity limits; it binds both pilot point hashes, the reference-environment hash, and the fitting method.
...
| Concurrency | Single user; v2.0.0 executes sequentially and removes `parallel.*` (OQ-037). | Reconsider only when the accepted 1M workflow exceeds 12 hours. | No worker protocol in v2; the run-level lock (OQ-027) still refuses a second concurrent invocation against the same target with exit 3. |
...
| Memory | Whole-run metadata is bounded-linear; provisional design guardrails are 8 GiB at 1M, 6 KiB/file, and a 20% linearity band. | Numeric values are not binding until the external uninstrumented 100k pilot and revision two. | Binding runs measure each stage in a fresh subprocess; thresholds load from `docs/scale-evidence/thresholds.json`. |
| Runtime | The complete installed 1M workflow must finish within 12 hours on the accepted reference environment. | Release qualification and scale-sensitive changes repeat the 1M tier. | Exceeding the bound fails qualification and reopens concurrency design; it never enables legacy config. |
...
Binding scale stages run in fresh subprocesses without `PYTHONTRACEMALLOC`, `-X tracemalloc`, or equivalent allocation tracing. The harness builds and hashes the candidate wheel from a clean checkout, invokes only that wheel for the 100k/1M tiers, validates every artifact passed between stages, and records public-safe evidence without hostnames, usernames, absolute paths, device identifiers, or document content. Python allocation peaks are diagnostic-only and never share a run with binding RSS measurement. Before corpus materialization, deterministic per-filesystem byte/inode accounting must include the recipe, expected artifact/log growth, tool backups when selected, largest staging file, and a 25% margin.
...
| FR-019 | Existing cooperative watchdog and oversize coverage — `tests/test_discovery.py::TestWatchdog`, `tests/test_planning.py::TestWatchdog`, `tests/test_watchdog.py`, and `tests/test_planning.py::TestFactGates::test_oversize_file__skipped_with_reason`; DMR-08 adds docs/comment assertions for the sequential native-call limitation and aggregate heartbeat silence semantics. | Partial (v1 cooperative behavior complete; DMR-08 Task 2/8 honesty and liveness sync pending) |
| NFR-001 | DMR-08 qualification suite: strict evidence/reference/threshold model, schema-parity, privacy, conservation, exact-fit/rounding, immutable-byte hashing, safe relative loading, threshold derivation, and provenance tests in `tests/test_scale_evidence.py` and `tests/test_schemas.py`; Linux mount-topology/classification, per-filesystem byte/inode capacity, exact reference comparison, and child/global swap contracts in `tests/test_scale_resources.py`; deterministic streamed recipe/disposition, exact byte/inode budget, race-safe materialization, private request/result, environment sanitization, process lifecycle, RSS, and swap contracts in `tests/test_scale_corpus.py` and `tests/test_scale_stage_runner.py`; 1,000-file source-tree regression in the default gate; accepted external uninstrumented 100,000-file pilot/scheduled evidence; accepted file-size matrix; accepted installed-wheel 1,000,000-file evidence satisfying corpus conservation, expected findings, peak/slope/linearity thresholds, reference environment, and 12-hour runtime. The historical `tests/test_scale.py` tracemalloc run is non-binding and will be replaced. | Partial (revision-one contract and Tasks 2-5 evidence/resource/corpus/supervisor contracts complete; orchestration, default guard, pilot, revision-two thresholds, and 1M evidence pending) |
...
| OQ-038 | What exact nested public records, artifact-key vocabularies, evidence-reference hashing rule, and threshold rounding semantics complete the approved DMR-08 qualification contract? | Proceeding on strict aggregate-only records with finite artifact keys; explicit mutually exclusive external-RSS/allocation-diagnostic fields; identifier-free per-filesystem preflight and conservation/finding totals; safe relative evidence identities hashed over exact file bytes; exact rational least squares with intercept; 25% peak and non-negative-slope headroom rounded upward; explicit provisional 20% linearity; passing installed-wheel tiers require exact wheel provenance and four completed stages; environment binding is a comparison verdict rather than model validity. | No | owner | DMR-08 pilot and threshold revision | Open; implementation assumption recorded in `docs/open-questions.md` |
