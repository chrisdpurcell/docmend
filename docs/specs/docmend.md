...
The library is a directory tree of more than 100,000 `.txt` and HTML files accumulated over decades, exhibiting every condition listed in §1: mixed encodings and newline styles, broken formatting, garbage text, corruption, non-descriptive names, and undetected near-duplicates. There is no inventory, no metadata, and no naming convention. Existing duplicate-detection tools fail because of noise and drift in the text. As of MS-3 (2026-07-06) the repository holds the foundation (build/tooling scaffold, CI, the `docmend` CLI entry point with global flags, strict TOML config loading, the logging framework, the NFR-005 purity enforcement), the discovery layer — `docmend scan` produces the DR-001 inventory read-only with FR-012 filters, and the four OQ-004 artifact schemas are pinned — and the planning layer: `docmend plan` produces the reviewable DR-002 plan (fact-gate ladder, content pass, collisions, C.4 provenance) over the FR-007–FR-009 pure transforms (encoding, newline, whitespace). Scan, plan, **apply, and restore** are now live: the writer layer (FR-005 safety gate, backups, manifest, atomic writes) lets `docmend apply` turn a reviewed plan into mutated files with a reversible manifest, and `docmend restore` undoes an apply run by replaying that manifest. MS-4 (unattended operation) is complete: `docmend verify` runs read-only over converted output — UTF-8-decodability, LF-only, frontmatter-where-present, manifest after-hash, and report↔manifest accounting checks (FR-014, FR-016, IR-004); an interrupted apply resumes via `--resume-run-id`/`--resume-manifest`, reconciling the plan against prior manifest records per adr-0006 (FR-013); and re-runs are idempotent in all three duplicate shapes (FR-017). MS-5 (production readiness) is complete as of v1.0.0 (2026-07-07): the seeded 100k-file scale test passes within a bounded memory ceiling (NFR-001), the FR-019 per-file watchdog bounds discovery/detection/transform work, the §18.7 documentation deliverables (README, runbooks, config reference) are shipped, and the adr-0017 release workflow publishes tagged GitHub Releases. The remaining §19 MS-5 items that require the real library — the §18.4 staged rollout and the weird-corpus expansion from real scan findings — are owner actions performed with the released tool. Post-release (2026-07-07): v1.0.1 shipped the issue-#15 partial-undo-trap console surfaces (rev 0.22), and the cross-repo alignment hardening (rev 0.24, `adr-0018-doc-processing-repository-boundary`) landed the FR-011 same-run collision invariant, the DR-004 write-ahead intent record (manifest schema 1.3) with §13.5-contained resume reconciliation, and excluded-directory pruning at scan — all ahead of the owner's first real-library mutation campaign. The 2026-07-10 comprehensive review (18 lenses; synthesis in `docs/codex-reviews/`) then confirmed seven rollout-blocking safety defects (DMR-01..07: same-plan backup collisions, artifact-destination clobber, manifest trust, unjournaled commit windows, false-clean verify, and mutation-time identity races); rev 0.26 recontracts the safety core against them (ADRs 0019–0021 plus amendments; design record `docs/superpowers/specs/2026-07-10-safety-core-remediation-design.md`), targeting **v2.0.0** with a clean manifest/backup format break. The staged real-library write rollout is paused behind that remediation.
...
docmend has no database; its persistent data model is (a) the four JSON run artifacts — five with the optional rev 0.26 `verify-report` (durable verification evidence, adr-0005 amendment) — and (b) the frontmatter embedded in converted documents. Exact JSON Schemas are pinned before implementation (OQ-004; pinned at MS-1 in `src/docmend/schemas/` — inside the package so the installed wheel can validate artifacts, with `docs`-side rationale in that directory's README); the binding shapes are outlined in DR-001–DR-004. For each artifact: identity is carried by the OQ-004 identity fields — run-ID, per-action ID, and `docmend.id` — with the run (timestamp + source root) plus per-file source path as the natural key; provenance requirements are the recorded source hashes and config snapshots that let any historical result be reproduced and explained; retention is indefinite until user purge (§18.6).
...
| FR-005 | Safety-gate refusal tests per missing strategy — `tests/test_gate.py` (refusal-per-missing-strategy set), `tests/test_cli_apply.py::test_apply_write_without_strategy__gate_refuses_exit_3`. | Partial (v1 contract Complete at MS-3; rev 0.26 adds the sealed `WriteSafetyContext` engine boundary — pending v2 implementation) |
| FR-006 | Backup + manifest recording and full-restore drill — `tests/unit/writer/test_backup.py`, `tests/test_apply.py::test_write_rewrite_in_place__utf8_lf_and_manifest`, `tests/test_restore_drill.py::test_restore_drill__manifest_replay_reproduces_original_corpus`. | Partial (v1 contract Complete at MS-3; rev 0.26 adds the write-once run/action/role BackupStore + plan-wide output ledger — the DMR-01 collision regression is pending v2 implementation) |
...
| FR-013 | Kill-and-resume end-to-end test — `tests/test_resume.py` (engine: interrupt mid-batch, resume completes remainder, corpus + union-of-manifests identical to uninterrupted control; already-applied / ERR-002 changed-output / ERR-002 missing-output / lost-trailing-record arms), `tests/test_cli_resume.py` (CLI: `--resume-run-id`/`--resume-manifest` incl. repeatable multi-resume chain, wrong-tree refusal, dry-run preview). Intent-record window arms (rev 0.24) — `tests/test_resume.py`: kill-after-publish completion, kill-before-final-record adoption, unclobbered-target re-execution, tampered target/source ERR-002, §13.5 out-of-root record-path refusal (source + target), dry-run preview; verify inertness `tests/test_verify.py::test_reconcile_ignores_intent_records`. | Partial (v1 contract Complete — MS-4; rev 0.24 rename_and_rewrite kill window closed via manifest 1.3 intents; rev 0.26 recontracts to journal-every-mutation with durable identities, chain lineage, and the full adjudication table — pending v2 implementation) |
| FR-014 | Verify command seeded-defect tests — bad-encoding, CRLF, hash-mismatch, missing-output, invalid-frontmatter, and report/manifest accounting drift each caught with exit 1; a clean corpus, a clean manifest, and clean cross-artifact accounting each exit 0 — `tests/test_cli_verify.py`, `tests/test_verify.py`, `tests/test_frontmatter.py`. | Partial (v1 contract Complete — MS-5 frontmatter-where-present + report/manifest accounting; rev 0.26 recontracts against the confirmed false-clean set and adds `verify --plan` — pending v2 implementation) |
...
| FR-019 | Watchdog-timeout termination tests — scan leg `tests/test_discovery.py::TestWatchdog::test_scan__slow_classification_recorded_as_timeout_batch_continues`, content-pass leg `tests/test_planning.py::TestWatchdog::test_content_pass__slow_transform_recorded_as_timeout` (a slow fixture is recorded with reason `timeout` while the batch continues), context-manager unit tests `tests/test_watchdog.py`; oversize plan-time skip test — `tests/test_planning.py::TestFactGates::test_oversize_file__skipped_with_reason`. | Complete (MS-5: in-process SIGALRM watchdog per DEV-002; MS-2 oversize plan-time guard) |
| NFR-001 | 100k-file synthetic corpus run; memory bound assertion — `tests/test_scale.py::test_scale_corpus__pipeline_totals_and_bounded_memory` (corpus seeded per OQ-032/`adr-0015-test-corpus-and-anonymization`, generated at test time, never committed; slow-marked and `DOCMEND_SCALE` opt-in; tracemalloc peak asserted against a per-file-count budget). | Complete (MS-5) |
...
| IR-004 | `verify` flag/sidecar-discovery tests; proven read-only — `tests/test_cli_verify.py` (exit 0/1/2 taxonomy, `--manifest`/`--run-id` resolution and their mutual exclusion, read-only assertion over corpus + manifest). | Partial (v1 contract Complete at MS-4; rev 0.26 adds the required Report input, lineage binding, and repeatable chain flags — pending v2 implementation) |
...
| IR-007 | Artifact round-trip tests (JSON documents; NDJSON manifest per record). Inventory half at MS-1: write→read→identical model, atomicity, ERR-008 failure modes — `tests/test_inventory_artifact.py`; plan half at MS-2 — `tests/test_plan_artifact.py`, `tests/test_schemas.py`; append `tests/test_report_artifact.py`, `tests/unit/writer/test_manifest.py`. | Partial (v1 contract Complete at MS-3; rev 0.26 adds the destination guard, `.docmend/` carve-out tests, and `O_EXCL` staging — pending v2 implementation) |
| IR-008 | `restore` drill test: manifest replay restores pre-apply hashes — `tests/test_restore.py`, `tests/test_restore_drill.py`. | Partial (v1 contract Complete at MS-3; rev 0.26 adds chain validation before path access, header-root locking, inverse journaling/convergence, and the zero-match selector finding — pending v2 implementation) |
...
| DR-003 | Report schema validation; summary-vs-outcome reconciliation tests — `tests/test_report_artifact.py` (schema + reconciliation). | Partial (v1 contract Complete at MS-3; rev 0.26 adds `not-attempted`, its totals extension, and the attempt-lineage fields — pending v2 implementation) |
| DR-004 | Manifest schema validation; mechanical-restorability test — `tests/unit/writer/test_manifest.py`, `tests/test_restore_drill.py` (mechanical restorability). | Partial (v1 contract Complete at MS-3; rev 0.26 recontracts to manifest 2.0 — header envelope, chain links, identities, two-scope lifecycle, 1.x rejection — pending v2 implementation) |
...
| DEV-002 | FR-019 (§7.1), ERR-009 (§12.1) | The FR-019 watchdog is realized in v1 as an in-process `signal.setitimer(ITIMER_REAL)` SIGALRM alarm scoped per file (module `docmend.watchdog`), not the "process-level … terminates" form FR-019 describes. On overrun it raises inside the same process; the offending file is recorded `timeout` and the batch continues (behaviourally identical to the required outcome). | v1's engine is sequential-in-process (adr-0007 keeps it single-process until profiling justifies a pool), so there is no worker process to terminate — a process-level kill has nothing to kill. Scope is still discovery/detection/transform only, never the writer. The process-level realization rides the deferred adr-0007 pool. | Approved (owner, 2026-07-07 post-release review; recorded at MS-5 closeout). |
