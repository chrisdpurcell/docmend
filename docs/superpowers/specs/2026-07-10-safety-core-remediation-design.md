...
- Immutable-field rule: an action's `intent` and terminal record must agree on `action_id`, `docmend_id`, `operation`, `original_path`, `target_path`, `before_sha256`, `after_sha256` (expected vs achieved), both backup references, and the pre-mutation identity fields below; only `result`, `error`, `seq`, `recorded_at`, and `published_identity` may differ. Divergence is a lifecycle violation.
...
- **Durable object identity (F4 round 3).** The identities the adjudication table consumes must survive the kill, so intent records persist them: `source_identity` — the `(st_dev, st_ino)` CommitBoundary captured from the descriptor it validated — and `target_identity`, the overwrite target's identity when one existed at commit (null otherwise). Restore-inverse intents record the identities they observed the same way. Terminal records add `published_identity`, the post-publish identity of the output — recorded because an atomic rewrite/publish mints a **new** inode via temp-and-replace, so the pre-mutation identity intentionally does not survive a completed publish; restore inverses verify the applied file against the terminal's `published_identity` before inverting it. Identity comparison is `st_ino` exact; `st_dev` must equal the recorded value or, uniformly for the whole chain, the current device of `source_root` (tolerating a remount/reboot renumbering without weakening per-file checks).
...
- The source opens once with `O_RDONLY | O_NOFOLLOW`; `fstat` captures `(st_dev, st_ino)`; the hash check, transform recompute, and backup all use bytes read from that descriptor. The captured identity is not process-local state: it is written into the action's intent record (`source_identity`, and `target_identity` when an overwrite target exists) so post-kill adjudication verifies against the same object the live boundary validated (F4 round 3).
...
Verify consumes Plan, **Report(s)**, ManifestChain, and BackupStore (F6) — matching ADR-0012's input contract (flags or sidecar discovery). **Attempt lineage (F6 round 3):** apply reports gain their own durable chain — `prior_report_sha256` (the sha256 of the same plan's previous attempt report, null for the first attempt) and `manifest_sha256` (the sha256 of the attempt's closed manifest, null when the attempt mutated nothing). Attempt order is the report chain — never caller order or wall-clock — and **report-only attempts occupy the chain**: an abort on the first action or an all-skip retry produces a report link with a null `manifest_sha256` and stays orderable. Binding is bidirectional: the report names its manifest by hash; the manifest's `run_id` must match exactly one report in the chain; and a manifest's `prior_manifest_sha256` must reference the newest manifest of an earlier attempt, so the two chains cannot disagree. Coverage binding: the report's `plan_ref` hash, the manifest headers' `plan_sha256`, and the plan artifact hash must agree. Every confirmed false-clean path becomes a finding:
...
- Schema versions: manifest 2.0; the report schema bumps for `not-attempted`, the totals extension, and the attempt-lineage fields (`prior_report_sha256`, `manifest_sha256`); the plan and inventory schemas are unchanged; `verify-report` is a new schema.
