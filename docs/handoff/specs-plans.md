...
| Plans | `docs/superpowers/plans/` | Active safety-core remediation plans: A (foundations, implemented), B (manifest 2.0, implemented), C (commit boundary, in review). The MS-2/MS-3 plans were pruned after v1.0.0 shipped (per `docs/repo-hygiene.md` — completed plans are deletable when the overall project ships); the safety-core plans stay until the v2.0.0 release PR lands. |
