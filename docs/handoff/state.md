...
- **Now active: MS-4 Unattended operation** — resume (FR-013 over the landed seq/fsync/AOF manifest), `verify` (FR-014), idempotency (FR-017). First task (fully spec'd): restore lock-key gap fix — manifest 1.2 `source_root` field + restore lock rekey (OQ-036). Then the deferred final-review minors (list: `sessions/2026-07.md`).
